...
export default async function Home() {
  ...
  const { data: pokemons, error } = await supabase
    .from('master_pokemon')
    .select('pokedex_id, name_ja, form_name_ja, type1, type2, base_hp, base_atk, base_def, base_spa, base_spd, base_spe')
    .limit(20)
    ...
    <main className="min-h-screen p-8 bg-gray-50 text-gray-900">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h2 className="text-xl font-semibold mb-4 text-gray-800">マスターデータ連携テスト (上位20件)</h2>
          ...
          {!pokemons || pokemons.length === 0 ? (
            <p>データがありません。Supabaseのシードが実行されているか確認してください。</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {pokemons.map((p) => {
                const displayName = p.form_name_ja ? `${p.name_ja} (${p.form_name_ja})` : p.name_ja;
                return (
                  <div key={`${p.pokedex_id}-${p.form_name_ja}`} className="p-4 rounded-lg bg-gray-50 border border-gray-200">
